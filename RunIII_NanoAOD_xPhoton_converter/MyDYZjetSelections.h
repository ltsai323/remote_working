#ifndef __MyDYZjetSelections_h__
#define __MyDYZjetSelections_h__

namespace DYZjetSelections
{
  template<typename T>
  ...
  TString IsolatedElectrons();
  TString JetRequirement();

  TString FindGoodj<F2> 
};



// detail inside the same file
#define __MyDYZjetSelections_C__
#ifdef __MyDYZjetSelections_C__
#include <numeric>

TString DYZjetSelections::IsolatedMuons()
{ return ""; }
TString DYZjetSelections::IsolatedElectrons()
{ return ""; }
TString DYZjetSelections::JetRequirement()
{ return "nJet>1"; }

template <typename T>
auto DYZjetSelections::FindGoodPhotons(T &df)
{ return df.Define("goodPhoton","Photon_pt > 180 && Photon_pt<1700 && abs(Photon_eta) < 2.5").Filter("Sum(goodPhoton)>=1"); }

TString DYZjetSelections::EventPassedPhoHLT() // event based, not object based
{ return "HLT_Photon200==1"; }
TString DYZjetSelections::PhotonPreselection()
{
...
TString DYZjetSelections::PhotonSaikatSelection()
{ return "Photon_pt > 180 && Photon_pt<1700 && abs(Photon_eta) < 2.5"; }


TString DYZjetSelections::LeadingPhotonSaikatSelection_Selection()
{
  std::vector<const char*> usedCuts({
      "HLT_Photon200==1",
...
#endif // __MyDYZjetSelections_C__
#endif // __MyDYZjetSelections_h__
...
//ROOT::VecOps::RVec<float> DYZjetSelections::EffectiveArea_ChIso(const ROOT::VecOps::RVec<float>& phoETA)
//std::vector<float> DYZjetSelections::EffectiveArea_ChIso(const ROOT::VecOps::RVec<float>& phoETA)
//  float* EffectiveArea_ChIso(const ROOT::VecOps::RVec<float>& phoETA)
//{
...
// //ROOT::VecOps::RVec<float> DYZjetSelections::EffectiveArea_ChIso(const ROOT::VecOps::RVec<float>& phoETA)
// //std::vector<float> DYZjetSelections::EffectiveArea_ChIso(const ROOT::VecOps::RVec<float>& phoETA)
// float* DYZjetSelections::EffectiveArea_ChIso(const ROOT::VecOps::RVec<float>& phoETA)
// {
//   //ROOT::VecOps::RVec<float> effAreas(phoETA.size());
//   //std::vector<float> effAreas(phoETA.size());
//   const short arrSize(phoETA.size());
