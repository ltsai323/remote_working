...
FILE_IDENTIFIER = 'collect_initinfo.py'
DEBUG_MODE = True
def BUG(mesg):
    if DEBUG_MODE:
...
# File path to the bash script

# Dictionary to store parsed values

def read_bashvar(bashSCRIPT:str):
    file_path = bashSCRIPT
    values = {}
    # Read the file and extract the desired variables
    with open(file_path, 'r') as file:
        for line in file:
            ...
            if '=' in line:
                ...
                # Convert numeric values to float, keep as string if conversion fails
                try:
                    value = float(value)
                except ValueError:
                    pass

                # Store the parsed value in the dictionary
                values[key] = value

    ### convert values to yaml content format
    nC = { 'value': values['initC'], 'error': -1, 'composition': -1 }
    nB = { 'value': values['initB'], 'error': -1, 'composition': -1 }
    nL = { 'value': values['initL'], 'error': -1, 'composition': -1 }
    return { 'nC':nC, 'nB':nB, 'nL':nL }
...
def mainfunc(
    ...
    #    exit()
    try:
        content = read_bashvar(fitinfoYAML)
    except FileNotFoundError as e:
        BUG(f'[skipped] File "{ fitinfoYAML }" not found because the fitting failed. skip it')
        exit()
...
def testfunc_read_bashvar():
    value = read_bashvar('PNet_bin2_originalBDTfit/bin_0_0_3/bashvar_sideband.sh')
...
if __name__ == "__main__":
    #testfunc_Entry()
    #testfunc_read_bashvar()

    import sys
    from collections import namedtuple
    InArgs = namedtuple('InArgs', 'bashvar_datasideband out_csv data_era pEtaBin jEtaBin pPtL pPtR')
    args = InArgs(*sys.argv[1:])

    mainfunc(
            args.bashvar_datasideband,
            args.out_csv,
            args.data_era,
