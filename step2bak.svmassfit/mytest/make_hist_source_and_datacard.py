...
def maked_datacard_and_hists( inFILE:str,
    ...
    #gethist = lambda hNAME: iFile.Get(output_hist_definitions[hNAME].iname)
    #def get_hist_to_local(hNAME): locals()[hNAME] = iFile.Get(output_hist_definitions[hNAME].iname)
    #hDATA = gethist('hDATA')
    #hFAKE = gethist('hFAKE')
    #hLjet = gethist('hLjet')
    #hCjet = gethist('hCjet')
    #hBjet = gethist('hBjet')
    ## set variable into locals
