...
_channle_=$3
#_channle_=`read_yaml_to_bash.py $histSOURCE_gjet channel`
OUT_FITINFO=`printf 'fitinfo_%s.yaml' $_channle_`
...
### Use sideband fit to extract the initial value of the C/B/L fractions
 ...
 _TMP_INPUT_HIST=`read_yaml_to_bash.py $histSOURCE_gjet  outputROOT`
 _TMP_DATACARD=`  read_yaml_to_bash.py $histSOURCE_gjet outputDATACARD`


 ###### created file from extract_data_entries.py
 _TMP_BASHVAR=bashvar_${_channle_}.sh

 ### [pre process] grab init values
 extract_data_entries.py \
   $_TMP_INPUT_HIST 'gjet/data_obs' \
   $_TMP_BASHVAR \
   $initVAL_gjet \
 ...
 _TMP_INPUT_WORKSPACE=ws_${_channle_}.root
 ...
 _TMP_MULTIDIMFITTEST=multidimfitTest_${_channle_}.root
 _TMP_HIGGSCOMBINETEST=higgsCombineTest.MultiDimFit.mH120_${_channle_}.root
