...
#define MAX_GENJET 80
  ...
  for(iEvent = 0; iEvent< nEntries; ++iEvent){
    ...
    for(int ij=0; ij< nPhoton; ++ij)
    {
      ...
      {
        if(abs(Photon_eta[ij])<1.4442 && Photon_pfChargedIsoPFPV[ij]<1.7 && Photon_pfChargedIsoWorstVtx[ij]<10) reco_photon_in_signalregion_or_datasideband = true;
        if(abs(Photon_eta[ij])>1.566  && Photon_pfChargedIsoPFPV[ij]<1.5 && Photon_pfChargedIsoWorstVtx[ij]<10) reco_photon_in_signalregion_or_datasideband = true;
      }
      else  // is data sideband
      {
        if(abs(Photon_eta[ij])<1.4442 && Photon_pfChargedIsoPFPV[ij]>7.0) reco_photon_in_signalregion_or_datasideband = true;
        if(abs(Photon_eta[ij])>1.566  && Photon_pfChargedIsoPFPV[ij]>7.0) reco_photon_in_signalregion_or_datasideband = true;
      }

      if(Photon_pfChargedIsoWorstVtx[ij]>20) continue;
      if(Photon_pfChargedIsoPFPV[ij]>20) continue;
      if(Photon_hoe[ij]>0.05) continue;
      if(abs(Photon_eta[ij])<1.4442&&Photon_sieie[ij]>0.015) continue;
