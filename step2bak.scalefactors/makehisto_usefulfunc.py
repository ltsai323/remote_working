...
class histCollection:
    ...
    def WriteAllHists(self, fOUT,
        ...
        fOUT.cd()
        def ww(obj, writeOVERFLOWbin=False, writeNORMALIZEDhist=False):
            if isinstance(obj, Iterable):
                for o in obj:
                    ww(o, writeOVERFLOWbin,writeNORMALIZEDhist)
            else:
                if writeOVERFLOWbin:
                    max_bin = obj.GetNbinsX()
                    obj.SetBinContent(1, obj.GetBinContent(0) + obj.GetBinContent(1) )
                    obj.SetBinContent(max_bin, obj.GetBinContent(max_bin) + obj.GetBinContent(max_bin+1) )

                    obj.SetBinContent(0, 0)
                    obj.SetBinContent(max_bin+1, 0)
                obj.Write()

                if writeNORMALIZEDhist:
                    norm_name = obj.GetName() + "_norm"
                    hnorm = obj.Clone(norm_name)
                    hnorm.Scale(1./hnorm.Integral())
                    hnorm.Write()
                
            
        for hname, hinst in vars(self).items():
            ww(hinst, writeOVERFLOWbin, writeNORMALIZEDhist)
