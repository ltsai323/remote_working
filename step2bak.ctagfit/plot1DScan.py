...
my_stored_val = {
        ...
        'errUp': val_nom[1],
        'errDn': abs(val_nom[2]),
        'error': val_nom[1] if val_nom[1] > abs(val_nom[2]) else abs(val_nom[2])
        }
with open(args.output + '.yaml', 'w') as fOUT:
    import yaml
    write_dict = {
            my_stored_val['varname']: {
                'value': my_stored_val['value'],
                'error': my_stored_val['error'],
                'errUp': my_stored_val['errUp'],
                'errDn': my_stored_val['errDn'],
                }
            }
    yaml.dump(write_dict, fOUT)
    print(f'[YamlOutput] {args.output}.yaml generated')
...
print(f'[ExtracdtInfo] {fixed_name} = {textfit}')
